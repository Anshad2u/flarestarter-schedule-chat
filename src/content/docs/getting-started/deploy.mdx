...
```bash
npx wrangler d1 create softfactory-db-prod          # D1 数据库
npx wrangler kv namespace create CACHE               # KV(会话/缓存)
npx wrangler r2 bucket create softfactory-files-prod # R2 对象存储(头像上传)
```
...
- `BETTER_AUTH_URL` —— 设为你的线上域名,例如 `https://softfactory.example.com`。首次只发
  workers.dev 时还没有自定义域,可先填可预测的 `https://softfactory-production.<你的子域>.workers.dev`,
  之后绑定自定义域再改回来。
...
# 等价于:npx wrangler d1 migrations apply softfactory-db-prod --env production --remote
```
...
默认环境**(worker 名 `softfactory`、全 0 占位绑定),不是生产。环境是在 build 时烘进
